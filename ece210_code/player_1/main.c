...
#define LOCAL_ID				0x32					// Local ID
#define REMOTE_ID				0x33				// Remote ID
...
main(void)
{
  uint32_t 	x_pos = X_START;		// x_pos holds the x position of the right corner of the image
	...
	bool			data_send = 0;
	uint32_t  turn = 1; // Set to 1 for player 1 and 0 for player 2
	int 			test = 0;	
	char 			msg[30];
	int 			start;
	uint8_t		game_over = 0;
  ...
  ece210_wireless_init(LOCAL_ID,REMOTE_ID);
	...
	ece210_lcd_print_string("Don't let the ball", 215, 170, LCD_COLOR_BLUE, LCD_COLOR_BLACK);
	ece210_lcd_print_string("touch the bottom!", 215, 190, LCD_COLOR_BLUE, LCD_COLOR_BLACK);
	...
	while(1)
	{
		...
		if (ece210_wireless_data_avaiable()) {
			...
			if (message_check == round_over){
				...
				ece210_lcd_print_string("Don't let the ball", 215, 170, LCD_COLOR_BLACK, LCD_COLOR_BLACK);
				ece210_lcd_print_string("touch the bottom!", 215, 190, LCD_COLOR_BLACK, LCD_COLOR_BLACK);
		...
		while(start) {
			if(AlertButtons) {
				...
				if (btn_down_pressed() || btn_left_pressed() || btn_right_pressed() || btn_up_pressed()){
					...
					ece210_lcd_print_string("touch the bottom!", 215, 190, LCD_COLOR_BLACK, LCD_COLOR_BLACK);
					ece210_lcd_print_string("GAME START!", 170, 170, LCD_COLOR_GREEN, LCD_COLOR_BLACK);
					ece210_wireless_send(game_start);
...
//		}
	...
	}	// end wireless data while loop

	if (data_send) { // Send if hit the top
